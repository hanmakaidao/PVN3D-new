...
import sys
sys.path.append("./train/YCBVideoTest/")
import torch
...
from train.YCBVideoTest.ycb_video_evaluator import YCBVideoSemanticEvaluator
...
def logger(log_name="YCB-Video", log_name_with_time=False):
...
def model_fn_decorator(
    criterion, criterion_of, test=False, loss_weight=None
):
    modelreturn = namedtuple("modelreturn", ["preds", "loss", "acc"])
    teval = TorchEval()

    def model_fn(
        model, data, epoch=0, is_eval=False, is_test=False, finish_test=False, iou_evaluator=None, scene_no=0
    ):
        if finish_test:
            teval.cal_auc()
            iou_evaluator.evaluate()
            return None
        if is_eval:
            model.eval()
        with torch.set_grad_enabled(not is_eval):
            ...
            if is_test:
                ...
                iou_evaluator.process(inputs, outputs)
...
class Trainer(object):
    ...
    def eval_epoch(self, d_loader, is_test=False):
        ...
        iou_evaluator = YCBVideoSemanticEvaluator()
        ...
        if is_test:
            self.model_fn(
                self.model, data, is_eval=True, is_test=is_test, finish_test=True, iou_evaluator=iou_evaluator
            )
...
if __name__ == "__main__":
    ...
    logger = logger(log_name_with_time=False)
    ...
    if args.checkpoint is not None:
        checkpoint_status = load_checkpoint(
            model, optimizer, filename=args.checkpoint[:-8], logger=logger
        )
